'''
Copyright (C) 2019 Serafym Smei
(https://github.com/s-smei)
Modified 2019 by Błażej Sewera
(https://github.com/jazzsewera)
- added trend lines for plots

Usage:
    python3 plot_file_trend.py <shape_name>__alg*.txt
'''
import sys
import re
...
def get_id(in_file):
    return in_file.split('__')[-1][0]
    # [0] is name of test file, [-1] is '<id>.txt', [-1][0] is '<id>'
    ...
    with open(in_file, "r") as f:
        ...
        for line in text:  # Search for a specific line
            if re.search("number of vertices", line):
                ...
                item = line.split()[-2]  # last item is 'sec', we need a number
                y = to_float(item)
                y_full.append(y)
                count += 1
...
if __name__ == "__main__":  # in terminal:
                            # python3 plot_file_trend.py <name>__alg*.txt
    results = []
    ...
    separated_results = {}
    # Key is 'file name'.
    # If 2 files or more with the same name, they would be held together.
    ...
    for key in separated_results:
        ...
        for measurement in separated_results[key]:
            ...
            # Trend lines start at 1000 in x axis
            # and end with a plot from the tests
            trend = np.polyfit(measurement['x'], measurement['y'], 2)
            linear_sp = np.linspace(1000, measurement['x'][-1], 100)
            plt.plot(
                measurement['x'],
                measurement['y'],
                '-',
                linear_sp,  # x axis for trend lines
                np.poly1d(trend)(linear_sp),  # trend function
                '--'
            )
